...
function StatusBadge({ status, statusLabels }: { status: IPStatus; statusLabels: Record<IPStatus, string> }) {
  const conf = {
    assigned: { label: statusLabels.assigned, cls: 'text-green-700 bg-green-50 border-green-200', icon: <CheckCircle size={12} /> },
    pending: { label: statusLabels.pending, cls: 'text-amber-700 bg-amber-50 border-amber-200', icon: <AlertTriangle size={12} /> },
    risk: { label: statusLabels.risk, cls: 'text-red-700 bg-red-50 border-red-200', icon: <AlertTriangle size={12} /> },
  }[status]
  return (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white border border-line rounded-xl p-4 shadow-sm hover:shadow-md transition">
          <div className="text-muted text-xs mb-1">{t.ipRegistry.totalAssets}</div>
        ...
        <div className={`border rounded-xl p-4 shadow-sm hover:shadow-md transition ${riskCount > 0 ? 'bg-red-50 border-red-200' : 'bg-white border-line'}`}>
          <div className="text-muted text-xs mb-1">{t.ipRegistry.risks}</div>
        ...
        <div className={`border rounded-xl p-4 shadow-sm hover:shadow-md transition ${pendingCount > 0 ? 'bg-amber-50 border-amber-200' : 'bg-white border-line'}`}>
          <div className="text-muted text-xs mb-1">{t.ipRegistry.inProgress}</div>
      ...
      {riskCount > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <div className="text-red-700 font-semibold text-sm mb-1">{t.ipRegistry.riskAlertTitle}</div>
            <div className="text-red-700/80 text-xs">
              <span className="font-mono">{riskCount}</span> {t.ipRegistry.riskAlertDesc}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border border-line rounded-2xl overflow-hidden shadow-sm">
        <div className="flex items-center justify-between px-6 py-4 border-b border-line">
          <h2 className="text-ink font-semibold tracking-tightest">{t.ipRegistry.tableTitle}</h2>
          {isAdmin && (
            <button
            ...
            >
              <Plus size={16} /> {t.ipRegistry.addAsset}
            </button>
          )}
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-line">
                <th className="text-left text-xs font-semibold uppercase tracking-[0.05em] text-muted px-6 py-3">{t.ipRegistry.colAsset}</th>
                <th className="text-left text-xs font-semibold uppercase tracking-[0.05em] text-muted px-4 py-3">{t.ipRegistry.colType}</th>
                <th className="text-left text-xs font-semibold uppercase tracking-[0.05em] text-muted px-4 py-3">{t.ipRegistry.colOwner}</th>
                <th className="text-left text-xs font-semibold uppercase tracking-[0.05em] text-muted px-4 py-3">{t.ipRegistry.colContractors}</th>
                <th className="text-left text-xs font-semibold uppercase tracking-[0.05em] text-muted px-4 py-3">{t.ipRegistry.colStatus}</th>
                <th className="text-left text-xs font-semibold uppercase tracking-[0.05em] text-muted px-4 py-3">{t.ipRegistry.colRisk}</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={asset.id} className="border-b border-line hover:bg-brand-surface transition-colors">
                  ...
                  <td className="px-4 py-4">
                    {isAdmin ? (
                      ...
                      >
                        <StatusBadge status={asset.status} statusLabels={t.ipRegistry.statusLabels} />
                      </button>
                    ) : <StatusBadge status={asset.status} statusLabels={t.ipRegistry.statusLabels} />}
                  </td>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white border border-line rounded-2xl p-6 w-full max-w-md shadow-md">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-ink font-semibold">{t.ipRegistry.modalTitle}</h3>
              <button onClick={() => setShowAdd(false)} className="text-muted hover:text-ink"><X size={18} /></button>
            </div>
            <div className="space-y-4">
              {([
                { label: t.ipRegistry.fieldName, key: 'name', placeholder: 'ML Model v3' },
                { label: t.ipRegistry.fieldOwner, key: 'owner', placeholder: 'Компания или ФИО' },
                { label: t.ipRegistry.fieldContractors, key: 'contractors', placeholder: 'Если есть — укажите' },
                { label: t.ipRegistry.fieldNotes, key: 'notes', placeholder: 'Дополнительная информация' },
              ...
              <div>
                <label className="text-muted text-xs mb-1 block">{t.ipRegistry.fieldType}</label>
                <select
                  className="w-full bg-white border border-line rounded-lg px-3 py-2 text-ink text-sm focus:outline-none focus:border-brand-blue"
                  value={newAsset.type}
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowAdd(false)} className="flex-1 border border-line text-muted py-2.5 rounded-xl text-sm hover:bg-brand-surface transition-colors">{t.common.cancel}</button>
              <button onClick={addAsset} className="flex-1 bg-brand-blue hover:opacity-85 text-white py-2.5 rounded-xl text-sm font-medium transition-colors">{t.common.add}</button>
            </div>
          </div>
        </div>
      )}
