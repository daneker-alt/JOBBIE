    ...
    <div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              <div className="text-gray-500 text-xs mb-1">Всего активов</div>
              <div className="text-2xl font-bold text-[#0B2D6B]">{assets.length}</div>
            </div>
            <div className={`border rounded-xl p-4 ${riskCount > 0 ? 'bg-red-500/5 border-red-500/20' : 'bg-white border-gray-200'}`}>
              <div className="text-gray-500 text-xs mb-1">Риски</div>
              <div className={`text-2xl font-bold ${riskCount > 0 ? 'text-red-400' : 'text-green-400'}`}>{riskCount}</div>
            </div>
            <div className={`border rounded-xl p-4 ${pendingCount > 0 ? 'bg-yellow-500/5 border-yellow-500/20' : 'bg-white border-gray-200'}`}>
              <div className="text-gray-500 text-xs mb-1">В процессе</div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h2 className="text-[#1F2937] font-semibold">Активы IP</h2>
              <button
                onClick={() => setShowAdd(true)}
                className="flex items-center gap-2 bg-[#0B2D6B] hover:bg-[#0B2D6B] text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors"
              >
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left text-gray-400 font-medium px-6 py-3">Актив</th>
                    <th className="text-left text-gray-400 font-medium px-4 py-3">Тип</th>
                    <th className="text-left text-gray-400 font-medium px-4 py-3">Владелец</th>
                    <th className="text-left text-gray-400 font-medium px-4 py-3">Подрядчики</th>
                    <th className="text-left text-gray-400 font-medium px-4 py-3">Статус</th>
                    <th className="text-left text-gray-400 font-medium px-4 py-3">Риск</th>
                  </tr>
                </thead>
                <tbody>
                  {assets.map(asset => (
                    <tr key={asset.id} className="border-b border-gray-100 hover:bg-gray-100/30 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <Shield size={14} className="text-gray-400 shrink-0" />
                          <div>
                            <div className="text-[#1F2937] font-medium">{asset.name}</div>
                            {asset.notes && <div className="text-gray-400 text-xs mt-0.5">{asset.notes}</div>}
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        <span className={`text-xs border px-2 py-0.5 rounded-full ${typeColors[asset.type] || 'text-gray-500 bg-gray-100 border-gray-200'}`}>
                          {asset.type}
                        </span>
                      </td>
                      <td className="px-4 py-4 text-gray-700">{asset.owner}</td>
                      <td className="px-4 py-4 text-gray-500">{asset.contractors || '—'}</td>
            ...
            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
              <div className="bg-white border border-gray-200 rounded-2xl p-6 w-full max-w-md">
                <div className="flex items-center justify-between mb-5">
                  <h3 className="text-[#0B2D6B] font-semibold">Добавить IP актив</h3>
                ...
                <div className="space-y-4">
                  {([
                    ...
                    <div key={key}>
                      ...
                      <input
                        className="w-full bg-gray-100 border border-gray-200 rounded-lg px-3 py-2 text-[#1F2937] text-sm focus:outline-none focus:border-[#0B2D6B]"
                ...
                <div className="flex gap-3 mt-6">
                  <button onClick={() => setShowAdd(false)} className="flex-1 border border-gray-200 text-gray-500 py-2.5 rounded-xl text-sm hover:border-slate-500 transition-colors">Отмена</button>
                  <button onClick={addAsset} className="flex-1 bg-[#0B2D6B] hover:bg-[#0B2D6B] text-white py-2.5 rounded-xl text-sm font-medium transition-colors">Добавить</button>
                </div>
              </div>
            </div>
          )}
