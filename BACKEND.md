...
- **Клиент** (`client`) — видит свой дашборд **только для просмотра** + проходит Legal Scan.
- **Сотрудник** (`admin`) — редактирует риск-скоры (ползунки), статусы документов (клик по бейджу), сохраняет изменения.
